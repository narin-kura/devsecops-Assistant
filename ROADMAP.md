...
- **K8s manifest / Helm chart generation — done, 2026-08-16.** Closes the
  loose end left from the first Containerization pass. Added to the same
  module rather than a new specialist, per the roadmap's original framing
  ("Dockerfile / Compose / K8s manifest / Helm chart generation" as one
  specialist's scope): `core/modules/containerize/k8s.py`
  (`generate_k8s_manifests()` for a plain Deployment+Service pair,
  `generate_helm_chart()` for a minimal installable chart — both reuse
  `container_profiles.get_container_profile()` for service name/port, the
  same profile Dockerfile generation already builds), new
  `templates/k8s/` and `templates/helm/` template trees, two new
  `agent_tools.py` tools (`generate_kubernetes_manifests`,
  `generate_helm_chart_files`), wired into the existing Containerization
  specialist (no new specialist agent) and the CLI (`k8s` subcommand,
  `--helm` to switch modes). Links a `kubernetes` registry entry.
  Real implementation wrinkle: a Helm chart's `templates/*.yaml` files are
  themselves Go-templates Helm re-renders at install time
  (`{{ .Values.x }}`) — the *same* delimiter Jinja2 uses by default.
  Wrapping every Go-template expression in `{% raw %}` (as the earlier
  GitHub Actions template bug fix did for a *different*-delimiter case,
  `${{ }}`) would repeat that exact failure mode for a same-delimiter
  case, where it's far easier to miss one. Instead, those two templates
  render through a Jinja `Environment` configured with
  `variable_start_string="[["`/`variable_end_string="]]"`, so `{{ }}`
  isn't special to Jinja there at all and Go-template syntax passes
  through untouched by construction — verified with a regression test
  asserting `{{ .Values.x }}` survives verbatim while `[[ service_name ]]`
  gets substituted. Image reference defaults to `<service-name>:latest`
  (matching `containerize`'s own suggested build command) since there's no
  way to infer a real registry path; overridable via `--image` (manifests)
  or `values.yaml` (Helm). No liveness/readiness probes generated by
  default — a wrong guess there causes real operational pain
  (CrashLoopBackOff-adjacent), so omitting is the safer default,
  consistent with this project's "never hard-fail, degrade gracefully"
  posture. 16 new tests. **Phase 2 is now fully complete.**
